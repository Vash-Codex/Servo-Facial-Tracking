...
def detect_arduino_port (default ="COM3"):
    ...
    preferred_terms =(
    "arduino","ch340","ch341","wch","usb serial","usb-serial",
    "cp210","cp2102","cp2104","silicon labs","ftdi","ft232","usb modem",
    "cdc","pro micro","leonardo","nano","uno","mega",
    )
...
def main ():
    ...
    if USE_ARDUINO :
        if serial is None :
            ...
            ports_to_try =[]
            if COM_PORT and COM_PORT !="AUTO":
                ports_to_try .append (COM_PORT )
            ...
            for port_candidate in ports_to_try :
                try :
                    arduino =serial .Serial (port_candidate ,BAUD ,timeout =0 )
                    time .sleep (2 )
                    print (f"[OK] Connected to Arduino on {port_candidate}.")
                    break 
                except Exception as exc :
                    print (f"[WARN] Failed connecting on {port_candidate}: {exc}")
